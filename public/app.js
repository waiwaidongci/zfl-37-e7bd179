...
function renderDrawerStatusOptions(detail) {
  const statuses = new Set(scoringStatuses && scoringStatuses.length ? scoringStatuses : stages);
  for (const item of detail.items || []) {
    if (item.status) statuses.add(item.status);
  }
  drawerStatusFilter.innerHTML = '<option value="">全部状态</option>' +
    Array.from(statuses).map(s => '<option value="'+escapeHtml(s)+'">'+escapeHtml(s)+'</option>').join('');
}

function openBatchDrawer(batchId) {
  api('/api/batches/' + encodeURIComponent(batchId)).then(detail => {
    ...
    renderDrawerStatusOptions(detail);
    drawerStatusFilter.value = '';
    drawerSearch.value = '';
    renderDrawerItems();
    batchDetailDrawer.style.display = '';
  }).catch(err => {
    alert('加载批次详情失败：' + (err.message || '未知错误'));
  });
}
...
function renderDrawerItems() {
  ...
  drawerItemList.innerHTML = visible.map(item => {
    const scoreBadge = item.latestScore !== null
      ? '<span class="pill ' + statusClass(item.status || '') + '">评分 ' + item.latestScore + '</span>'
    ...
    const storageBadge = item.storage
      ? '<span class="pill">' + escapeHtml(item.storage) + '</span>'
    ...
    return '<div class="drawer-item">' +
      '<div class="drawer-item-header">' +
        ...
        '<span class="pill ' + statusClass(item.status || '') + '">' + escapeHtml(item.status || '待试磨') + '</span>' +
      '</div>' +
      '<div class="drawer-item-meta">' +
        '<span>烟料：' + escapeHtml(item.smokeSource || '-') + '</span>' +
        '<span>年限：' + (item.ageYears ?? '-') + ' 年</span>' +
      '</div>' +
      '<div class="drawer-item-badges">' + scoreBadge + storageBadge + retestBadge + taskBadge + '</div>' +
      '<div class="drawer-item-actions">' +
        (canCreateTask
          ? '<button class="secondary gold" data-drawer-create-task="' + escapeHtml(item.id) + '">创建试磨任务</button>'
          : '') +
        (item.hasActiveTask
          ? '<button class="secondary" data-drawer-goto-task="' + escapeHtml(item.activeTaskId) + '">查看任务</button>'
          : '') +
      '</div>' +
    '</div>';
  }).join('');
...
async function createSingleTaskFromDrawer(itemId) {
  ...
  try {
    ...
    alert('任务创建成功！');
    const batchId = currentBatchDetail && currentBatchDetail.id;
    await load();
    if (batchId) openBatchDrawer(batchId);
  } catch (err) {
...
async function createBatchTasksFromDrawer(targetStatus, itemIds = null) {
  ...
  const assignee = prompt('请输入负责人', '');
  if (assignee === null) return;
  const noteInput = prompt('任务备注（可选）', '');
  if (noteInput === null) return;

  const payload = { scheduledDate, assignee, note: noteInput || '' };
  ...
  try {
    const result = await api('/api/batches/' + encodeURIComponent(currentBatchDetail.id) + '/tasks', {
      method: 'POST',
...
if (drawerClose) drawerClose.onclick = closeBatchDrawer;
if (batchDetailDrawer) {
...
if (drawerStatusFilter) drawerStatusFilter.onchange = renderDrawerItems;
if (drawerSearch) drawerSearch.oninput = renderDrawerItems;
if (drawerCreateTasksBtn) {
